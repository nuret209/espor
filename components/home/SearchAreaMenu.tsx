            ...
            (submenu, index) =>
              <div key={index} className='flex flex-col'>
                ...
                {'submenus' in submenu ? submenu.submenus.map(subsubmenu =>
                  (!search || subsubmenu.name.toLowerCase().includes(search.toLowerCase())) && <div key={subsubmenu.name} className='hovericon group text-[#c5c5c5] lg:text-gray-700 flex items-center justify-between px-3 cursor-pointer py-2 mx-2 transition-all duration-200 lg:hover:bg-[#f7f7f7] text-paragraph-md'>
                    {subsubmenu.name} <RiArrowRightSLine className='lg:opacity-0  text-[#525252] duration-200 transition-all group-hover:opacity-100' />
                  </div>) :
                  (!search || submenu.name.toLowerCase().includes(search.toLowerCase())) && <div key={submenu.name} className='hovericon group text-[#c5c5c5] lg:text-gray-700  flex items-center justify-between px-3 cursor-pointer py-2 mx-2 transition-all duration-200 lg:hover:bg-[#f7f7f7] text-paragraph-md'>
